...
const HoverComponent = ({films}) => {
    ...
    const fetchFilmById = async (filmId) => {
      ...
      try {
          ...
          if (filmSnap.exists()) {
              console.log("Film data:", filmSnap.data());  
    ...
    return (
      <div className="absolute  inset-0 bg-slate-400 mx-7 pointer bg-opacity-60 flex flex-col justify-center items-center opacity-0 hover:opacity-100 transition-opacity duration-300 p-2">
        <div className="text-center ">
          ...
          <p className="mt-4 text-gray-600 flex gap-2 items-center justify-center cursor-pointer bg-gray-300 bg-opacity-80 py-2 px-3 rounded-2xl hover:bg-gray-200 font-semibold transition-all duration-300" onClick={() => fetchFilmById('36feGoxZXR1KuZIVtcKk')}>See more <span><Icon name="search" size={15} color="gray" /></span></p>
        </div>
      </div>
    );
  };
